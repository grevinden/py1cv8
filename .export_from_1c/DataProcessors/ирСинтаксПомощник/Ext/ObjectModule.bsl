...
// 
Функция ЗагрузитьОписаниеСвойстваВСтроке(Знач СтрокаОписания) Экспорт 
...
Функция ТекстовоеОписаниеСлова(Знач СтрокаСлова) Экспорт 
	НовыйАдрес = РаспаковатьЭлементАрхиваСинтаксПомощника(СтрокаСлова.ПутьКОписанию);
	Если НовыйАдрес <> Неопределено Тогда
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.Прочитать(ирОбщий.ПервыйФрагментЛкс(НовыйАдрес, "#"));
		ТекстДокумента = ТекстовыйДокумент.ПолучитьТекст();
