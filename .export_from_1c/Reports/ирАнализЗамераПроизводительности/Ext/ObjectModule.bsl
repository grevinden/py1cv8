...
Функция ВнешниеНаборыДанных() Экспорт 
	...
	Таблица.Колонки.Добавить("НомерПервойСтрокиМетода", Новый ОписаниеТипов("Число")); // - нужно для сортировки
	...
	Если ИзвлекатьИменаМетодов Тогда
		...
		Для Счетчик = 1 По НачальноеКоличество Цикл
			...
			СтрокаЗамера.Метод = мПлатформа.НайтиИмяМетодаСтрокиМодуля(СтрокаЗамера.Модуль, СтрокаЗамера.НомерСтроки, СтрокаЗамера.Текст, ТекстыМодулейКэш, СтароеИмяФайлаМодуляБезРасширения,,,,
				СтрокаЗамера.НомерПервойСтрокиМетода);
	...
	Для Каждого СтрокаЗамера Из Таблица Цикл
		...
		Если НРег(ирОбщий.ПоследнийФрагментЛкс(СтрокаЗамера.Модуль)) <> "epf" Тогда
			СтрокаЗамера.СсылкаСтроки = ирОбщий.СсылкаСтрокиМодуляЛкс(СтрокаЗамера.Модуль, СтрокаЗамера.НомерСтроки, СтрокаЗамера.Метод, СтрокаЗамера.НомерСтроки-СтрокаЗамера.НомерПервойСтрокиМетода);
		КонецЕсли; 
