...
Функция КакJson(Ответ, ПараметрыПреобразованияJSON = Неопределено) Экспорт
	...
	Исключение
		...
		ВызватьИсключение(КакИсключение(
		Ответ, НСтр("ru = 'Ошибка при десериализации JSON.'"))
...
Функция ОтправитьЗапрос(Сессия, ПодготовленныйЗапрос, Настройки) Экспорт
...
Функция НастройкиПодключения(Сессия, Метод, URL, ДополнительныеПараметры) Экспорт
...
Процедура Приостановить(ДлительностьОстановкиВСекундах)
	Если ирКэш.ЭтоФоновоеЗаданиеЛкс() Тогда
	 	ВызватьПаузу(ДлительностьОстановкиВСекундах);
	 	Возврат;
	КонецЕсли;
